...
def import_data_to_neo4j(neo4j_uri, neo4j_user, neo4j_password, data, create_node_query):
    ...
    for row in data:
        ...
        graph.run(create_node_query, row=row)
    ...
    "HOSPITALIZATION": {
        "query": """
            CREATE (:Hospitalization {
                ADMISSION_DATE: date({row[0]}),
                DISCHARGE_DATE: date({row[1]}),
